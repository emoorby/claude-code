...
input ENUM_TRIGGER_TIMEFRAME InpTriggerTimeframe = TRIGGER_M1; // Candle Close Trigger Timeframe
...
datetime g_lastTriggerBarTime = 0;    // Last bar time on trigger timeframe
bool     g_buyTradeOpened = false;    // Flag: buy trade opened today
bool     g_sellTradeOpened = false;   // Flag: sell trade opened today
...
void OnTick()
{
   ...
   // If range is identified, monitor for candle close breakouts
   if(g_rangeIdentified && g_tradesCount < InpMaxTradesPerDay)
   {
...
//+------------------------------------------------------------------+
//| Monitor for candle close breakout and place market orders        |
//+------------------------------------------------------------------+
void CheckCandleCloseBreakout()
{
   // Convert trigger timeframe enum to ENUM_TIMEFRAMES
   ENUM_TIMEFRAMES triggerTF;
   switch(InpTriggerTimeframe)
   {
      case TRIGGER_M1:  triggerTF = PERIOD_M1;  break;
      case TRIGGER_M5:  triggerTF = PERIOD_M5;  break;
      case TRIGGER_M15: triggerTF = PERIOD_M15; break;
      case TRIGGER_M30: triggerTF = PERIOD_M30; break;
      case TRIGGER_H1:  triggerTF = PERIOD_H1;  break;
      default:          triggerTF = PERIOD_M1;  break;
   }

   // Check if a new bar has closed on the trigger timeframe
   datetime currentBarTime = iTime(_Symbol, triggerTF, 0);

   // If no new bar has formed, skip the check
   if(currentBarTime == g_lastTriggerBarTime)
      return;

   // New bar formed - update the time
   g_lastTriggerBarTime = currentBarTime;

   // Get the close price of the most recently completed bar (bar 1)
   double closePrice = iClose(_Symbol, triggerTF, 1);

   Print("========================================");
   Print("New ", EnumToString(triggerTF), " bar closed at ", TimeToString(currentBarTime));
   Print("Close Price: ", closePrice, " | Range High: ", g_rangeHigh, " | Range Low: ", g_rangeLow);

   // Check for BUY signal (candle closed above range high)
   if(!g_buyTradeOpened && closePrice > g_rangeHigh &&
      (InpTradeDirection == TRADE_BOTH || InpTradeDirection == TRADE_BUY_ONLY))
   {
      Print("BUY signal detected - Close (", closePrice, ") > Range High (", g_rangeHigh, ")");

      // Run ATR filter if enabled
      bool filterPassed = CheckATRFilter(ORDER_TYPE_BUY, g_rangeSize);

      if(filterPassed)
      {
         Print("Filter PASSED - Placing market BUY order");
         ...
         if(lotSize > 0)
         {
            ...
            Print("Market BUY order details:");
            ...
            if(OrderSend(request, result))
            {
               ...
               {
                  Print("SUCCESS: Market BUY order executed. Ticket: ", result.order);
                  g_buyTradeOpened = true;
                  g_tradesCount++;
                  Print("Trade count increased to: ", g_tradesCount, "/", InpMaxTradesPerDay);
               }
               else
               {
                  Print("ERROR: Market BUY order failed. Return code: ", result.retcode,
            ...
            else
            {
               ...
               Print("ERROR: OrderSend failed for Market BUY. Error code: ", lastError);
            }
         }
         else
         {
      ...
      {
         Print("Filter FAILED - BUY trade not allowed");
      }
   }

   // Check for SELL signal (candle closed below range low)
   if(!g_sellTradeOpened && closePrice < g_rangeLow &&
      (InpTradeDirection == TRADE_BOTH || InpTradeDirection == TRADE_SELL_ONLY))
   {
      Print("SELL signal detected - Close (", closePrice, ") < Range Low (", g_rangeLow, ")");

      // Run ATR filter if enabled
      bool filterPassed = CheckATRFilter(ORDER_TYPE_SELL, g_rangeSize);

      if(filterPassed)
      {
         Print("Filter PASSED - Placing market SELL order");
         ...
         if(lotSize > 0)
         {
            ...
            Print("Market SELL order details:");
            Print("  Price: ", request.price);
            Print("  Stop Loss: ", request.sl);
            Print("  Take Profit: ", request.tp);
            Print("  Lot Size: ", request.volume);
            ...
            if(OrderSend(request, result))
            {
               if(result.retcode == TRADE_RETCODE_DONE)
               {
                  Print("SUCCESS: Market SELL order executed. Ticket: ", result.order);
                  g_sellTradeOpened = true;
                  g_tradesCount++;
                  Print("Trade count increased to: ", g_tradesCount, "/", InpMaxTradesPerDay);
               }
               else
               {
                  Print("ERROR: Market SELL order failed. Return code: ", result.retcode,
                        " (", GetRetcodeDescription(result.retcode), ")");
                  Print("  Error details: ", result.comment);
               }
            }
            ...
            {
               int lastError = GetLastError();
               Print("ERROR: OrderSend failed for Market SELL. Error code: ", lastError);
            }
         }
         else
         {
      ...
      {
         Print("Filter FAILED - SELL trade not allowed");
      }
   }
...
void CheckNewDay()
{
   ...
   if(time_struct.day != g_currentDay)
   {
      ...
      g_buyTradeOpened = false;
      g_sellTradeOpened = false;
      g_currentDay = time_struct.day;
...
bool CheckATRFilter(ENUM_ORDER_TYPE orderType, double rangeSize)
{
   ...
   // Determine trade direction for logging
   bool isBuyDirection = (orderType == ORDER_TYPE_BUY || orderType == ORDER_TYPE_BUY_STOP || orderType == ORDER_TYPE_BUY_LIMIT);

   // Detailed logging
   Print("========================================");
   Print("VOLATILITY & MOMENTUM FILTER:");
   Print("Breakout Type: ", (isBuyDirection ? "BUY" : "SELL"));
   Print("");
   ...
   Print("  ", (isBuyDirection ? "Bullish" : "Bearish"), " candles: ",
